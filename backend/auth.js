...
async function signup(req, res) {
    ...
    db.get(`SELECT * FROM users WHERE email = ?`, [email], async (err, existingUser) => {
        ...
        if (existingUser) {
            if (existingUser.verified === 1) {
                ...
                db.run(`UPDATE users SET first_name = ?, last_name = ?, password = ?, otp = ?, otp_expiry = ? WHERE email = ?`,
                    [first_name, last_name, hashedPassword, otp, otpExpiry, email], async (err) => {
                        if (err) {
        ...
        db.run(`INSERT INTO users (first_name, last_name, email, password, otp, otp_expiry, verified) 
                VALUES (?, ?, ?, ?, ?, ?, 0)`,
            [first_name, last_name, email, hashedPassword, otp, otpExpiry], async function(err) {
                if (err) {
